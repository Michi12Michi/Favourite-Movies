...
class AddMovieForm(FlaskForm):
	title = StringField("Insert a movie title", validators = [DataRequired()])
...
def add_movie():
	...
	if form.validate_on_submit():
		...
		return render_template("select.html", opts = results)
...
def find():
	movie_id = request.args.get("id")
	if movie_id:
		response = requests.get(url = f"{API_INFO_URL}/{movie_id}", params = {"api_key": API_KEY})
		response.raise_for_status()
		res = response.json()
		movie = Movie(
			title = res["title"],
			...
			img_url = f"{API_URL}{res['poster_path']}"
		)
